...
public class TongZhiDAO {
	...
	public boolean save(Tongzhi tongzhi) {
		create();
		String sql = "insert into Tongzhi(tzid,tztext,tztitle,umid,tzdate,status,tzType,additional)"  
			+ "values(?,?,?,?,?,?,?,?)";
		try {
			db.execSQL(
					sql,
	...
	public boolean updateStatus(int tzid) {
		create();
		String sql = "update Tongzhi set status = 0 where tzid = ?";
		try {
			db.execSQL(
					sql,
					new String[] { 
	...
	public boolean updateChatInfo(Tongzhi tongzhi) {
		create();
		String sql = "update Tongzhi set tzid = ?, tztext = ?,tztitle = ?,status = ?,tzType = ? where umid = ? and additional = ?"; 
		try {
			db.execSQL(
					sql,
					new String[] { "" + tongzhi.getTzid(),
							tongzhi.getTztext(),tongzhi.getTztitle(),
							""+tongzhi.getStatus(),
							""+tongzhi.getTzType(),
							"" + tongzhi.getUmid(),
	...
	public boolean deleteAllByUmid(int umid) {
		create();
		String sql = "delete from Tongzhi where umid = ? ";
		try {
			db.execSQL(sql,new String[] {"" + umid,"" + umid});
		} catch (Exception e) {
			return false;
		} finally {
			close();
	...
	public boolean deleteLiaoTianByUmid(int umid,String additional) {
		create();
		String sql = "delete from Tongzhi where umid = ? and additional = ? and tzType = 3";
		try {
	...
	public List<Tongzhi> findByUmid(int umid) {
		create();
		String sql = "select * from Tongzhi where umid = ? and tzid > 0 order by tzid desc";
	...
	public int findUnReadCount(int umid) {
		create();
		String sql = "select count(*) from Tongzhi where umid = ? and status = 1 and tzid > 0";
		cursor = db.rawQuery(sql, new String[]{""+umid});
		int i = 0;
		while (cursor.moveToNext()) {
			i = cursor.getInt(0);
	...
	public Tongzhi isHasChatInfo(int umid,String additional) {
		create();
		String sql = "select * from Tongzhi where umid = ? and additional = ? and tzType = 3 and tzid > 0 order by tzid desc";
		cursor = db.rawQuery(sql, new String[]{""+umid , additional});
	...
	/*public List<Tongzhi> findLiaoTianByIds(int umid, String additional) {
		create();
		String sql = "select * from Tongzhi where ((umid = ? and tzType = 3 and tzid != -1 and additional = ?) or (additional = ? and umid = ? and tzType = 3 and tzid = -1))  order by tzdate asc";
	...
	public Tongzhi findByTzid(int tzid) {
		...
		String sql = "select * from Tongzhi where tzid = ? ";
